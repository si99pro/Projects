...
import {
    ...
    CardHeader,
    IconButton,
    Skeleton, // Keep Skeleton import for other parts if needed
    Alert,
    Link as MuiLink,
    useTheme,
    useMediaQuery,
...
// AllMomentListSkeleton remains, but won't be used as fallback for AllMomentList anymore
...
function Dashboard() {
    ...
    // ... (derived state variables remain the same) ...
    ...
    // Skeleton Loader Structure (Still used for initial page load)
        ...
        <DashboardContentArea>
             <Grid container spacing={{ xs: 2, md: 3 }}>
                ...
                <Grid item xs={12} lg={4}> {/* Adjusted grid size */}
                    {/* Discussions Skeleton - Use the AllMomentListSkeleton here for initial structure */}
                    <StyledCard sx={{ display: 'flex', flexDirection: 'column' }}>
                        <CardHeader title={<Skeleton animation="wave" height={30} width="60%" />} sx={{ pb: 0 }}/>
                        <CardContent sx={{ flexGrow: 1, overflowY: 'auto', minHeight: 0, p: { xs: 2, sm: 3 } }}>
                            <AllMomentListSkeleton /> {/* Show skeleton during initial page load */}
                        </CardContent>
                    </StyledCard>
                </Grid>
            </Grid>
         ...
         <DashboardContentArea>
            ...
            <Grid container spacing={{ xs: 2, md: 3 }}>
                ...
                <Grid item xs={12} lg={4}> {/* Adjust grid size */}
                    {/* Discussions Card - Removed Suspense fallback */}
                    <StyledCard variant="outlined" sx={{ display: 'flex', flexDirection: 'column' }}>
                        <CardHeader title="Latest Discussions" titleTypographyProps={{ variant: 'h6', fontWeight: 500 }} sx={{ pb: 0 }}/>
                        <CardContent sx={{ flexGrow: 1, overflowY: 'auto', minHeight: 0, p: { xs: 2, sm: 3 } }}>
                            {/* Suspense without fallback handles lazy loading without showing skeleton */}
                            <Suspense>
                                <AllMomentList />
                            </Suspense>
                        </CardContent>
                 ...
                 {!loading && user && isDesktopLg && isChatExpanded && (
                     <StyledChatSidebar elevation={0} variant="outlined">
                         <Suspense fallback={<ChatSkeleton />}>
                              {/* Wrap PublicChat in a Box with padding for margin */}
                             <Box sx={{
                                 p: { xs: 1, sm: 2 }, // Add padding around the chat component
                                 height: '100%',
                                 display: 'flex',
                                 flexDirection: 'column',
                                 overflow: 'hidden' // Ensure content within padding scrolls if needed
                                 }}>
