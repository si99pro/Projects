/* eslint-disable no-unused-vars */
 ...
 // --- Helper Functions (formatDisplayValue, getInitials, debounce - unchanged) ---
 ...
 // --- Reusable Components (EditableField, ItemBox, Forms - unchanged from "Fresh Redesign") ---
 const EditableField = React.memo(({
     ...
     const formattedDisplayValue = formatDisplayValue(currentValue);
     return ( <Box onClick={!isEditing ? handleEditClick : undefined} sx={{ position: 'relative', py: 1, display: 'flex', alignItems: 'center', cursor: (readOnly || disabled || isEditing) ? 'default' : 'pointer', borderBottom: `1px solid var(--color-border, ${theme.palette.divider})`, '&:last-of-type': { borderBottom: 'none' }, transition: 'background-color 0.2s ease', '&:hover': { backgroundColor: (readOnly || disabled || isEditing) ? 'transparent' : 'action.hover', '& .edit-action-icon': { opacity: 1 } }, minHeight: '56px', }}> {IconComponent && ( <Box sx={{ mr: 2, color: 'action.active', display: 'flex', alignItems: 'center' }}> <IconComponent sx={{ fontSize: '1.3rem' }} /> </Box> )} <Box sx={{ flexGrow: 1 }}> {!isEditing ? ( <> <Typography variant="caption" color="text.secondary" display="block" sx={{ lineHeight: 1.2, mb: 0.2 }}>{label}</Typography> <Typography variant="body1" component="div" sx={{ fontWeight: 500, color: 'text.primary', wordBreak: 'break-word', whiteSpace: multiline ? 'pre-wrap' : 'normal' }}> {formattedDisplayValue} </Typography> </> ) : ( <TextField placeholder={placeholder} value={currentValue ?? ''} onChange={handleChange} onKeyDown={handleKeyDown} fullWidth multiline={multiline} rows={rows} variant="outlined" size="small" autoFocus disabled={disabled || isSaving || readOnly} sx={{ '& .MuiInputBase-root': { backgroundColor: 'var(--color-surface-variant, #f8f9fa)' } }} /> )} </Box> {!readOnly && !disabled && ( <Box sx={{ ml: 1 }}> {isEditing ? ( <Stack direction="row" spacing={0.5}> <Tooltip title="Save"> <span> <IconButton onClick={handleSaveClick} size="small" disabled={isSaving || currentValue === value} color="primary"> {isSaving ? <CircularProgress size={20} color="inherit" /> : <SaveIcon fontSize="small" />} </IconButton> </span> </Tooltip> <Tooltip title="Cancel"> <IconButton onClick={handleCancelClick} size="small" disabled={isSaving}> <CancelIcon fontSize="small" /> </IconButton> </Tooltip> </Stack> ) : ( <Tooltip title="Edit"> <IconButton size="small" onClick={handleEditClick} className="edit-action-icon" sx={{ opacity: 0, transition: 'opacity 0.2s ease' }}> <EditOutlinedIcon fontSize="small" /> </IconButton> </Tooltip> )} </Box> )} </Box> ); });
 const ItemBox = ({
     children, onEdit, onDelete, isEditing, isSaving }) => {
     const theme = useTheme(); return ( <Box sx={{ p: 2, mb: 1.5, display: 'flex', alignItems: 'flex-start', border: `1px solid var(--color-border, ${theme.palette.divider})`, borderRadius: 'var(--border-radius-medium, 8px)', bgcolor: 'var(--color-surface, #fff)', position: 'relative', overflow: 'hidden', transition: 'box-shadow 0.2s ease', '&:hover': { boxShadow: 'var(--mui-shadows-2)', '& .item-actions': { opacity: 1, transform: 'translateX(0)', } } }}> <Box sx={{ flexGrow: 1, mr: 1 }}>{children}</Box> {!isEditing && (onEdit || onDelete) && ( <Stack direction="row" spacing={0.5} className="item-actions" sx={{ position: { xs: 'relative', sm: 'absolute' }, top: { sm: theme.spacing(1.5) }, right: { sm: theme.spacing(1.5) }, opacity: { xs: 1, sm: 0 }, transform: { sm: 'translateX(10px)' }, transition: 'opacity 0.2s ease, transform 0.2s ease', backgroundColor: 'rgba(255, 255, 255, 0.8)', backdropFilter: 'blur(2px)', borderRadius: 'var(--border-radius-small, 4px)', p: 0.5, }}> {onEdit && ( <Tooltip title="Edit"> <IconButton size="small" onClick={onEdit} disabled={isSaving}> <EditOutlinedIcon fontSize="small" /> </IconButton> </Tooltip> )} {onDelete && ( <Tooltip title="Delete"> <IconButton size="small" onClick={onDelete} disabled={isSaving} color="error"> <DeleteOutlineIcon fontSize="small" /> </IconButton> </Tooltip> )} </Stack> )} </Box> ); };
 const FormContainer = ({ children, title, ...props }) => ( <Box component={Paper} elevation={0} variant="outlined" sx={{ mt: 2, p: { xs: 2, sm: 3 }, borderRadius: 'var(--border-radius-medium, 8px)', mb: 3, bgcolor: 'var(--color-surface-variant, #f8f9fa)' }} {...props}> <Typography variant="h6" sx={{ mb: 2.5, fontSize: '1.1rem', fontWeight: 600 }}>{title}</Typography> {children} </Box> );
 ...
 /**
  * Profile Page Component - Full Width Desktop Redesign (No Skeleton)
  */
 const Profile = () => {
   const theme = useTheme();
   ...
   const [loading, setLoading] = useState(true); // Still needed to know when data is ready
   const [error, setError] = useState('');
   const [snackbar, setSnackbar] = useState({ open: false, message: '', severity: 'success' });
   const [activeTab, setActiveTab] = useState(0); // Still used for mobile view
   ...
   const avatarSize = { xs: 80, sm: 100, md: 120 }; // Responsive Avatar Size
   ...
   useEffect(() => {
     ...
     const fetchUserProfile = async () => {
         if (!currentUser?.uid) {
             if (isMounted) { setError("Not logged in."); setLoading(false); }
             return;
         }
         // Don't reset state here if we want to avoid flashes
         // setLoading(true); // Keep this to track loading status
         setError('');
         ...
         try {
             ...
             if (isMounted) setLoading(false); // Set loading false only when done
         }
     };
     fetchUserProfile();
   ...
   // --- Save Handlers & Callbacks ---
    ...
    // --- Work/Edu Add/Edit/Cancel/Delete Handlers ---
   ...
   // --- Tab Change Handler (Only for Mobile View) ---
   ...
   // --- RENDER LOGIC ---
     ...
     // If still loading or critical error state before data load, show minimal feedback
     if (loading) {
         return (
     ...
     // Error State after attempting load
     if (error && !basicInfo) {
         return (
             <Box sx={{ width: '100%', maxWidth: '800px', margin: '40px auto', p: 3 }}>
                 <Alert severity="error" sx={{ mb: 2 }}>{error}</Alert>
                 {error.includes("Not logged in") && <Button variant="contained" onClick={() => navigate('/login')} sx={{mt: 1}}>Login</Button>}
             </Box>
         );
     }
     // Fallback if data isn't available for some reason after loading finished
     if (!basicInfo) {
   ...
   return (
     <Box sx={{ width: '100%', maxWidth: '1200px', margin: '0 auto', p: { xs: 1, sm: 2, md: 3 } }}>
       <CssBaseline />
         ...
         <Paper elevation={0} sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, alignItems: 'center', p: { xs: 2, sm: 3 }, mb: 3, bgcolor: 'var(--color-surface, #fff)', borderRadius: 'var(--border-radius-large, 12px)', border: `1px solid var(--color-border, ${theme.palette.divider})` }} >
             <Avatar
                 sx={{
                     ...
                     fontSize: { xs: '2rem', sm: '2.5rem', md: '3rem' }, // Relative font size
             ...
             <Box sx={{ textAlign: { xs: 'center', sm: 'left' }, width: '100%' }}>
                  ...
                  {/* Display fetch error if any, after loading is complete */}
                  {!loading && error && <Alert severity="warning" sx={{ mt: 2, width: 'fit-content', mx: { xs: 'auto', sm: 0 } }}>{error}</Alert>}
             </Box>
         </Paper>
 
 
         {/* --- MOBILE VIEW: HORIZONTAL TABS (XS, SM screens) --- */}
         <Box sx={{ display: { xs: 'block', md: 'none' } }}>
             <Paper elevation={0} sx={{ mb: 3, bgcolor: 'var(--color-surface, #fff)', borderRadius: 'var(--border-radius-large, 12px)', border: `1px solid var(--color-border, ${theme.palette.divider})`, overflow: 'hidden' }} >
         ...
         {/* --- DESKTOP VIEW: FULL WIDTH STACKED CONTENT (MD+ screens) --- */}
         <Box sx={{ display: { xs: 'none', md: 'block' } }}>
              <Paper elevation={0} sx={{ bgcolor: 'var(--color-surface, #fff)', borderRadius: 'var(--border-radius-large, 12px)', border: `1px solid var(--color-border, ${theme.palette.divider})`, p: { md: 3, lg: 4 } }} >
